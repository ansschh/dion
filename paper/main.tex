...
% Allow compilation without figure files
\let\OrigIncludegraphics\includegraphics
\renewcommand{\includegraphics}[2][]{%
  \IfFileExists{#2}{\OrigIncludegraphics[#1]{#2}}{%
    \fbox{\parbox{0.8\textwidth}{\centering\vspace{2em}\textit{Figure not yet generated:}\\[0.5em]\texttt{#2}\vspace{2em}}}%
  }%
}
...
\begin{tabular}{llccc}
\toprule
\textbf{Optimizer} & \textbf{LR} & \textbf{rank\_fraction / $\alpha$} & \textbf{Final Loss} & \textbf{Status} \\
\midrule
...
% ─────────────────────────────────────────────────────────────────
\section{Single-Node Results (4 x H200)}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/fig3_loss_vs_tokens.png}
    \caption{Training loss vs.\ tokens consumed (single-node, 4 GPUs, default HPs, 10k steps). AdamW converges fastest, followed by Muon. Dion and Dion2 converge more slowly at their default lr=0.02.}
    \label{fig:loss_tokens}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/fig5_sweep_heatmaps.png}
    \caption{Hyperparameter sweep results (24 configurations, 5k steps each). Clear pattern: lr=0.005 with high rank\_fraction/alpha is optimal for both Dion and Dion2.}
    \label{fig:sweep}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/fig6_scaling_comparison.png}
    \caption{Final loss comparison between single-node FSDP (4 GPU) and multi-node HSDP (8 GPU). Dion improves by 26$\times$ with HSDP gradient replication.}
    \label{fig:scaling}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/fig7_main_grid_multinode.png}
    \caption{Multi-node HSDP (8 GPU) loss vs.\ tokens. With gradient replication across nodes, all four optimizers converge to near-zero loss.}
